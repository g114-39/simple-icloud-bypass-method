...
if [ ! -e "$oscheck"/gaster ]; then
    ...
    curl -sLO https://nightly.link/verygenericname/gaster/workflows/makefile/main/gaster-"$oscheck-x86_64".zip
    unzip gaster-"$oscheck-x86_64".zip
    mv gaster "$oscheck"/
    rm -rf gaster gaster-"$oscheck-x86_64".zip
fi
...
pyimg4 im4p create -i work/ramdisk.dmg -o work/ramdisk.im4p -f rdsk
pyimg4 img4 create -p work/ramdisk.im4p -m work/IM4M -o sshramdisk/ramdisk.img4
